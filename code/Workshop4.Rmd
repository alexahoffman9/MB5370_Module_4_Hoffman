...
# 5.6 Downloading and loading the spatial dataset
  *Your hero professor has sent you the data files in a .zip format. The spreadsheet copepods_raw.csv has measurements of copepod species richness from around Australia. Copepods are a type of zooplankton, perhaps the most abundant complex animals on the planet and an important part of ocean food-webs. *
...
Customizing:
Adding in a color scale to display richness as a continuous variable.
```{r message=FALSE, warning=FALSE}
library(viridis)
tm1 <- tm_shape(sdat) +
...
# 5.15 Customising copepod map
```{r message=FALSE, warning=FALSE}
tmap_mode("view")

tm_shape(shelf, bbox = sdat) + 
  tm_polygons(fill = "#1a5276") +
tm_shape(aus) + 
...
map <- tm_basemap("Esri.OceanBasemap") +
tm_shape(shelf, bbox = sdat) + 
  tm_polygons(fill = "#1a6b8a", fill_alpha = 0.4) +
tm_shape(aus) + 
  tm_polygons(fill = "#d4a96a", fill_alpha = 0.6) +
tm_shape(sdat) + 
  tm_dots(fill = "richness_raw",
          fill.scale = tm_scale_continuous(values = "plasma"),
          fill.legend = tm_legend(title = "Species Richness"),
...
tmap_save(map, filename = "copepod_richness_map.html")
```
